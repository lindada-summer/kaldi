...
def get_args():
    ...
    parser.add_argument("--trainer.min-transition-prob", type=float, dest='min_transition_prob',
                        default=0.0,
                        help="Minimum transition probabilityn")
    parser.add_argument("--trainer.transition-probs-update-interval", type=int,
                        dest='transition_probs_update_interval',
                        default=0,
                        help="Interval for updating transition model in percent.")
...
#            rand_prune=args.rand_prune)
    ...
    stats_dir = args.dir + '/trans_stats'
    for iter in range(num_iters):
        ...
        if args.stage <= iter:
            logger.info("Iter {}/{}    Percent {}".format(iter, num_iters-1, percent))
            ...
            if args.transition_probs_update_interval > 0:
                try:
                    os.makedirs(stats_dir)
                except OSError as e:
                    if e.errno == errno.EEXIST and os.path.isdir(stats_dir):
                        pass
                    else:
                        raise e
                chain_opts += ' --write-trans-stats-prefix={}/st-'.format(stats_dir)
            if percent > 0 and percent % args.transition_probs_update_interval == 0:
                common_lib.execute_command(
                    "steps/nnet3/chain/sum_trans_stats.sh --percent {p} {d}".format(d=stats_dir, p=percent))
            if percent >= 2*args.transition_probs_update_interval and args.transition_probs_update_interval > 0:
                chain_opts += ' --trans-probs-filename={}/percent-{}.stats --min-transition-prob={}'.format(
                    stats_dir,
                    (percent/args.transition_probs_update_interval)
                    * args.transition_probs_update_interval, args.min_transition_prob)
            ...
            if args.transition_probs_update_interval > 0:
                common_lib.execute_command(
                    "steps/nnet3/chain/sum_trans_stats.sh --iter {it} {d}".format(d=stats_dir, it=iter))
    ...
    if args.stage <= num_iters + 1:
        ...
        v = [line.strip() for line in open('{}/percent-{}.stats'.format(stats_dir, 100-args.transition_probs_update_interval))]
        v = [float(i) for i in v[0].split()[1:-1]]
        for i in range(0, len(v), 2):
            bisum = v[i] + v[i+1];
            v[i]/=bisum; v[i+1]/=bisum
            if min(v[i], v[i+1]) < args.min_transition_prob:
                if v[i] < v[i+1]:
                    v[i] = args.min_transition_prob
                    v[i+1] = 1.0 - args.min_transition_prob
                else:
                    v[i] = 1.0 - args.min_transition_prob
                    v[i+1] = args.min_transition_prob
        sumv = sum(v)
        v = [sumv / i for i in v]
        vs = [str(i) for i in v]
        open('{}/tr-priors.vec'.format(args.dir), 'wb').write("[ " + " ".join(vs) + " ]\n")
        common_lib.execute_command(
            "nnet3-am-adjust-priors {d}/final.mdl {d}/tr-priors.vec {d}/final.mdl".format(d=args.dir))
