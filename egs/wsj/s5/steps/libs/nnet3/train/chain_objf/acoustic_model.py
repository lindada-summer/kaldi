...
def train_one_iteration(dir, iter, srand, egs_dir,
    ...
    compute_train_cv_probabilities(
        ...
        leaky_hmm_coefficient=leaky_hmm_coefficient, run_opts=run_opts, chain_train_opts=chain_train_opts)
...
def compute_train_cv_probabilities(dir, iter, egs_dir, l2_regularize,
    ...
    common_lib.background_command(
        """{command} {dir}/log/compute_prob_valid.{iter}.log \
                nnet3-chain-compute-prob {co} --l2-regularize={l2} \
        ...
        """.format(command=run_opts.command, dir=dir, iter=iter, model=model,
                   ...
                   xent_reg=xent_regularize, co=chain_train_opts,
                   egs_dir=egs_dir))
...
def combine_models(dir, num_iters, models_to_combine, num_chunk_per_minibatch_str,
                   ...
                   sum_to_one_penalty=0.0, comb_opts=''):
    ...
    common_lib.execute_command(
        """{command} {combine_queue_opt} {dir}/log/combine.log \
                nnet3-chain-combine {co} --num-iters={opt_iters} \
                --l2-regularize={l2} --leaky-hmm-coefficient={leaky} \
                ...
                "ark,bg:nnet3-chain-copy-egs ark:{egs_dir}/train_subset_all.cegs ark:- | \
    ...
    compute_train_cv_probabilities(
        ...
        l2_regularize=l2_regularize, xent_regularize=xent_regularize,
        leaky_hmm_coefficient=leaky_hmm_coefficient,
        run_opts=run_opts, chain_train_opts=comb_opts)
